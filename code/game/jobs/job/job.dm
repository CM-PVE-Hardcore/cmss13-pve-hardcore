/datum/job
	...
	var/gear_preset_tertiary //Additional-Additional gear preset for different things
...
/datum/job/proc/equip_job(mob/living/M)
	...
	if(ishuman(M))
		...
		var/obj/structure/machinery/cryopod/pod = locate() in get_turf(human)
		if (pod)
			pod.go_in_cryopod(human, silent = TRUE)
		else
			for(var/cardinal in GLOB.cardinals)
				pod = locate() in get_step(human, cardinal)
				if(pod)
					pod.go_in_cryopod(human, silent = TRUE)
					break
